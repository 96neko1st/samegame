...
public class Field {
  ...
  public Field(int w, int h, Piece[] ps, Minim minim)
  {
    ...
    // 背景画像データ読み込み
    imgBackground = loadImage("background3.png");
  ...
  public void create() {
    ...
    // フィールドのデータの各要素に駒の種類の番号を割り当てる
    for (int y=0; y<rows; y++) {
      for (int x=0; x<cols; x++) {
        // ランダムに配置する
        data[y][x] = (int)random(pieces.length);
  ...
  public void draw() {
    ...
    textSize(48);
    text("turn:"+ sc.player_id[sc.current_turn], 650, 470);
  ...
  private void drawScore() {
    // 1の位から順に描画していく
    for (int i=0; i<sc.player_number; i++) {
      int x = 552;  // 1桁目の開始座標
      int s = score[i];
      for (int j=0; j<5; j++) {
        // スコアを10で割った余りに対応する数字の画像を表示
        image(imgScore[s % 10], x + (180 * i), 108);
  ...
  public boolean removePiece() {
    ...
    if (count > 0) {
      ...
      // スコア計算: (消した数-2)の2乗
      score[sc.current_turn] += pow(count-2, 2);
    }
